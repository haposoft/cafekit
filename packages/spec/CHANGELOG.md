...
- **`cf:debug` writes a report sized to the depth it states, `cf:fix` accepts that report, and the `debugger` agent follows the skill's gates (`debug-proportional`, with `debug-eval-instrument`).** The report now carries a `**Depth:** quick/local | standard | incident/deep` line; Quick/local and Standard reports write the root-cause contract, the confirmed hypothesis, the verification plan and the fix direction, and only Incident/deep adds the timeline, the other hypotheses, the elimination path and the recurrence-prevention handoff. The six `✓ Step` lines, the mermaid chart and the restated scout checklist are gone; the final answer opens with `## Debug Report`; Step 1 invokes the `scout` skill; confidence is `high` only when the cause was reproduced or observed at runtime; heavy reads on shared staging or production need the user's permission. `cf:fix --from-debug` requires the timeline and elimination path only for `incident/deep` (or no depth). The `debugger` agent uses the skill's report shape and, at the start of every investigation, points to its two gates. Measured with `claude plugin eval` after repairing the eval's proportion and confidence graders, ten runs per case, both arms on `claude` 2.1.281: a proportionate report 1/10 → 9/10 (sonnet) and 2/10 → 8/10 (opus); a `## Debug Report` heading 39/40 → 40/40 and 12/40 → 40/40; primary graders unchanged (39/40 and 40/40).
- **Three skills now live in the directory their public name says, and `cf:scout` is a short subroutine (`scout-subroutine`).** Claude Code shows a project skill under its directory name, not the frontmatter `name` — a session lists `develop`, not `cf:develop` — so callers asking for `cf:scout`, `cf:fix` and `cf:ask` pointed at names no Claude host shows; the skills appeared as `inspect`, `hotfix` and `question`. In its whole local history Claude Code never invoked scout, while Codex, which opens skill files by path, read it in 480 of 1956 sessions. The directories are renamed `scout`, `fix` and `ask`; the installer retires the old ones through `obsolete.skills`, removing only pristine managed copies, and CafeKit's catalog marks a user-modified old copy `retired_skill` instead of dropping the public name as a duplicate. Proved by a real upgrade from the previous install for Claude and Codex (`scripts/check-skill-rename.sh`): nothing changes under `--dry-run`, then the old directories go, the new ones arrive, and both catalogs list all three names. Typed commands become `/scout`, `/fix` and `/ask`. Scout itself went from 142 to 61 lines: the scope gate and no-scan lists are kept verbatim; the `Scout Report` template, used in 1 of 480 Codex sessions, is replaced by findings the caller folds into its own report (files as `path:line`, entrypoint and call path, blast radius, patterns, unknowns); a host that enables delegation on its own no longer counts as the user's permission; the two-phase delegation procedure moved to the reference and the unenforceable three-minute timeout is gone. The suite goes from 1382 to 1403 tests. A new eval case, `evals/scout/chan-doan-giam-gia`, has `/cf:debug` diagnose a threshold bug five calls deep in a 63-file project: every run diagnosed it correctly, but scout was invoked in only 2 of 5 sonnet runs even with the name visible — runs read debug's "Step 1: Scout" as a step to do themselves, which is a limit of the callers and is not fixed here. A user-modified old directory still shows in the host beside the new one.
- **`cf:develop` proves its verification runs before writing code, names the command that yields Base and Head, and was measured on a workspace where an honest run can close (`develop-repairs`).** The skill now runs the task's exact Verification Plan command once before changing anything and expects it to fail (except under `--flash` or when the command costs money or writes artifacts); a pass before any change, or a failure naming something other than the unmet Acceptance, is a blocker to raise rather than a defect to work around. A repair round is defined (one observed failure plus its repair and rerun), the Receipt is written last after proof and review both pass, the scout reads the test or probe that will judge the task, and `### Flash` is reduced to a pointer. The Receipt line now says Base and Head are copied from `node .claude/scripts/provenance.cjs … --json` and never typed or computed by hand: no skill, reference or gate message had named that command. `quality-gate.md` lets an artifact-pinned Receipt be rebound only when every declared hash matches, no file the proof reads changed, and every claim derives from those artifacts; otherwise it re-runs. The gate's `command_identity` accepts a Verification Plan with several top-level `- Command:` lines, binds the Receipt to the last one, and never counts an indented or sub-heading `Command`. The suite goes from 1366 to 1382 tests. **The instrument had to be repaired first.** Inside `claude plugin eval` runs `/usr/bin/git` (the Xcode stub) fails, so the `evals/develop` workspace had no usable git; an honest run could not derive Base and Head, while `receipt-day-du` (`Base:\s*\S+`) passed runs that invented them. The `develop-efficiency` close counts therefore measured willingness to invent binding, not develop quality. Scaffolds now commit the workspace and ship the provenance script, the grader requires a 40-hex Base, and git is resolved from Homebrew. The sandbox also denies `find` and `Glob` into a plugin's `references/`, so only `SKILL.md` text reliably reaches a run. Both skill versions were then measured on the repaired instrument, ten runs per cell: clean sonnet closed correctly 6/10 before and 9/10 after (pre-repair misses wrote Head as prose), clean opus 10/10 and 9/10; with a verification command broken for a reason unrelated to the code, opus stopped at it 10/10 in both versions but before touching any code 0/10 before and 10/10 after, and sonnet raised it 0/10 before and 4/10 after. Measured limits: sonnet closed the broken task in 16 of 20 runs, and every one of the 11 whose Receipt command was checked had substituted a working command for the plan's, which the real gate rejects and the eval cannot see (the other five traces were deleted before they were read); sonnet's verification runs in the broken cell did not fall (median 5 → 6); ten runs per cell does not separate one or two runs; the eval runs without hooks or the completion gate. $36.19 across the seventeen result directories of this packet.
- **`cf:brainstorm`'s description now covers the surfaces its body serves, and `cf:specs` drops a vague adjective (`skill-routing` follow-up).** `cf:brainstorm` previously advertised only the competing-designs case, leaving three body surfaces without a catalogue entry: open exploration, a single-path candidate that still needs a decision, and a bug that needs framing before diagnosis. The line now covers exploration and the single-path candidate, and sends a bug or failure that still needs its root cause elsewhere; `when_to_use` states the same conditions. In `cf:specs`, "hurried" is no longer an entry condition — hurry remains only as the reason not to skip the scope gate. Unmeasured: this edits the text the `skill-routing` measurement described, so its counts are stale until the suite runs again.
- **The two skill descriptions now say where an under-specified request goes, and one model stopped ignoring the door (`skill-routing`).** `cf:specs` says a short, vague or hurried request belongs there, because the scope gate is where the undecided questions get asked; `cf:brainstorm` says it is for an agreed outcome with two competing designs. Both lines state the same condition from opposite sides, and it is the condition the skill bodies already held. Measured on the same ambiguous request as before, same prompt bytes, same graders, same CLI build, twenty runs per model against a ten-run baseline: runs that invoked no skill at all fell from 9 of 20 to 4 of 40, two-sided Fisher p = 0.0057 against a resolution recorded before the run started. Read per model the result splits: sonnet went from 6 of 10 to 0 of 20 (p = 0.0004) and opus from 3 of 10 to 4 of 20 (p = 0.66, not separable from noise), so one model's unrouted behaviour is gone and the other's is unchanged as far as this can tell. The negative cases held, 0 of 12 invocations across a git question and a typo fix, which excludes an over-trigger rate above roughly 22% and no lower. `cf:brainstorm` was invoked 0 of 40 times, the same as the baseline, and that is not a result: the suite holds no case where an outcome is agreed and two designs compete. The four runs that still invoke nothing are all opus and all one shape — they report that the file-writing tools are disabled, then deliver the implementation as code to paste — so what moved is how many runs reach a door, not what an unrouted opus run does. $11.77 including judge cost.
- **`cf:specs` gained eight mechanisms, and what a model does with them was measured (`specs-a-plus`).** The plan template now carries a `## Decisions` table and a `Priority` column; a task carries ordered `## Steps` and a `## Failure Protocol`; verification is stated as a comparison with the Oracle rather than a judgement; GATE-SCOPE is asked in one message that lists every open item as `[NEEDS CLARIFICATION: <question>]` and always offers EXPAND, KEEP and CUT; Step 0 says request size is not a risk signal; and the author re-runs a sample of the plan's citations before GATE-REVIEW. Develop, `rules/workflow.md` and `spec-maker` now load the two new task sections. The nine-file bundle stays at 745/750 lines, the suite goes from 1353 to 1366 tests, and the semantic-weakening counterexamples from 23 to 30. Two eval cases were added: an ambiguous request, and a resumed transcript (`context.history_file`) that measures the packet a model writes after the user answers KEEP. Measured across sonnet and opus, three runs each — one sample, and a later sample of the same cases under the same skill gave 4 of 6 and 3 of 6 where this one gave 6 of 6 and 5 of 6, so read these as sample figures, not properties: the skill is invoked in 12 of 12 positive runs, stops at GATE-SCOPE in 11 of 12, never fires in 24 negative runs, and the resumed case passes ten of eleven graders 6 of 6 — the model writes `## Decisions`, `Priority`, `## Steps`, `## Failure Protocol` and `- Oracle:` unprompted. The 2026-09-17 miss where opus called "export customers to CSV" routine did not reproduce; in the two runs that stopped at the gate it classifies that request critical, while the third delivered code without classifying anything. Measured limits: with an ambiguous request the skill is invoked in only 4 of 6 runs; `moi-ac-co-task` fails whenever a plan writes its criteria as a range (`AC-01..AC-06`) rather than a list, which is a grader defect crossing both models; `khong-code` counts a prose line opening with the word "Export" as code, so one of the two failures on the export case is a grader artifact rather than written code; `khong-tu-chot` mixes a rubric that penalises the mandatory minimum-change proposal with one genuine failure; and every run put the whole feature in a single Tasks row. One real adherence gap: with Write and Edit withheld, opus announced it would hand over complete code to paste and delivered roughly 130 lines of implementation across five code blocks in chat — withholding the tools moved implementation into the answer rather than preventing it.
